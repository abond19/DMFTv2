...
def dH_mu_dm1_arr(q, m1, m2):
    """E[phi'(m1+G1)*phi(m2+G2)], broadcast over arbitrary-shaped arrays q, m1, m2.

    This is the *mixed* sigma'-sigma two-time kernel, H_mu_mix.
    It equals partial_m1 H_mu, i.e. d/dm1 of E[phi(m1+G1)*phi(m2+G2)].

    Physical origin: by Price's theorem on the router covariance C_d^v,
      partial_{C_d^v} H_{ss,d} = (a1/sqrt(2))^2 * E[phi(z^w(t)) phi(z^w(s))]
    But for M^cross_{R,Pv}, differentiating H_{sc,o} w.r.t. P^v_cross gives:
      partial_{P^v_cross} H_{sc,o} = (a1/sqrt(2)) * E[G_e sigma'(z^w_e(t)) sigma(z^w_{e'}(s))]
    which — after factoring out G_e — involves dH_mu_dm1 at the relevant covariance.

    Key separability result for H_{sc,o} (cross-cluster, cross-expert term):
      In H_{sc,o}(t,s), z^w_e(t) uses cluster-0 data and z^w_{e'}(s) uses cluster-1
      data, which are INDEPENDENT. Therefore Cov(z^w_e(t), z^w_{e'}(s)) = 0 EXACTLY,
      and dH_mu_dm1(q=0, m1, m2) = E[phi'(m1+G1)] * E[phi(m2+G2)] factorises as:
        E[phi'(m1+G)] = (1/sqrt(pi)) * exp(-m1^2/4)
        E[phi(m2+G)]  = erf(m2/2)
      The separable form used in MRPv_cross_rn is NOT an approximation — it is exact.

    Note: at P_cross = 0, m2 = kappa*P_cross = 0, and dH_mu_dm1(q, m1, 0)
    involves phi(G2) whose mean is 0, so E[phi(G2)] = 0 → M^cross ~ 0 at P_cross=0.
    """
    q  = np.clip(np.asarray(q,  float), -1+1e-10, 1-1e-10)
    m1 = np.asarray(m1, float); m2 = np.asarray(m2, float)
    if q.ndim == 0:
        return dH_mu_dm1(float(q), float(m1), float(m2))
    ZZ    = _ZZ.reshape((-1,) + (1,)*q.ndim)
    WW    = _WW.reshape((-1,) + (1,)*q.ndim)
    phip1 = np.sqrt(2./np.pi) * np.exp(-0.5*(m1+ZZ)**2)   # sigma'(m1+G1)
    phi2  = erf((m2 + q*ZZ) / np.sqrt(2.*(2. - q*q)))      # E[sigma(m2+G2)|G1=Z]
    return np.sum(WW * phip1 * phi2, axis=0)
